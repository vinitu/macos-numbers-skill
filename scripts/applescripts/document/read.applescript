...
on run argv
    ...
    try
        if (count of argv) is not 1 then error "Usage: osascript scripts/document/read.applescript <file-path>"

        set inputPath to my standardizePath(item 1 of argv)
        set {docRef, wasOpen} to my openDocument(inputPath)
        set resultPayload to {|file|:inputPath, |sheets|:my readSheets(docRef)}
...
on readSheets(docRef)
    ...
    repeat with sheetRef in sheetRefs
        ...
        repeat with tableRef in tableRefs
            set end of tablePayloads to my readTable(contents of tableRef)
        end repeat
